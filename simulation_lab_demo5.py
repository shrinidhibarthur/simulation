...
if st.session_state.step == 0:
    ...
    st.markdown('<div class="main-header">[LAB] Simulation Lab</div>', unsafe_allow_html=True)
    ...
    st.markdown("### [TARGET] Key Features & Capabilities")

    col1, col2 = st.columns(2)

    with col1:
        with st.container():
            st.markdown("**[ROCKET] Request to Results in Hours**")
        ...
        with st.container():
            st.markdown("**[CHART] Intelligent Scenario Generation**")
        ...
        with st.container():
            st.markdown("**[BOLT] Parallel Simulation Engine**")
    ...
    with col2:
        with st.container():
            st.markdown("**[TRENDING] Decision Dashboard & Analytics**")
        ...
        with st.container():
            st.markdown("**[TARGET] Automated Rollout Plans**")
    ...
    tab1, tab2, tab3 = st.tabs(["[CHART] Core Systems", "[AI] AI Architecture", "[CLIPBOARD] Integration Summary"])
...
└─────────────────┬───────────────────────────────────────┘
   ...
   └───────────────────────────────────────────────────────────┘
        ...
        with col2:
            ...
            with st.expander("[CLIPBOARD] Project & Workflow Management", expanded=False):
                st.markdown("""
...
└─────────────────┬───────────────────────────────────────┘
   ...
   └─────────┘ └───────┘                    └───────────┘
        ...
        st.markdown("#### [AI] LLM Use Cases in Simulation Lab")

        use_cases = pd.DataFrame({
            'Stage': [
                '💬 Request Entry',
                '❓ Clarification',
                '[CHART] Scenario Generation',
                '🔍 Data Quality',
                '[TRENDING] Results Interpretation',
                '[TARGET] Rollout Planning'
            ],
            'LLM Function': [
        ...
        with col2:
            st.markdown("**[TARGET] Recommended Setup**")
            st.info("""
    ...
    with tab3:
        st.markdown("#### [CHART] Integration Summary")
        ...
        st.markdown("#### [ROCKET] Phased Implementation Roadmap")
...
Example 2: Sticky Add-to-Bag CTA
Objective: Simulate and quantify the impact of a sticky Add-to-Bag CTA for Bras Category on mobile Product Detail Pages, measuring both UX performance and business KPIs.

Simulation Setup:
- Scenarios (Editable):
  - Baseline: Current PDP (no sticky CTA)
  - Variant A: Sticky CTA (lazy-loaded after 40 percent scroll)
- User Cohort: Domestic Logged-in Users (Regular shoppers on mobile web PDP - majority 4G and Wi-Fi traffic)
- Pages included: All Bras Collection pages
- Traffic Load: 1.2x normal campaign volume
- Device & Network Mix: 65 percent 4G, 35 percent Wi-Fi
- APIs Monitored: /product, /recommendation, /cart/add
"""
Goal: lift conversion and protect margin while maintaining delivery SLAs."""
    ...
    with col2:
        if st.button("[ROCKET] Submit Request", type="primary"):
            if request_text:
                st.session_state.step = 2
                # st.rerun()  # Commented to avoid compatibility issues
            else:
    ...
    st.title("[AI] AI Clarifier - Smart Questions")
    ...
    **Your Request:**
    ...
    with col2:
        ...
        <h4>[CLIPBOARD] Use Case Summary</h4>
    ...
    st.title("[CHART] Recommended Scenario Options")
    ...
    st.title("[GEAR] Simulation Configuration")
    ...
    st.title("[ROCKET] Running Simulation")
    ...
    # Telemetry metrics
    st.subheader("[CHART] Real-Time Simulation Telemetry")
    ...
    st.title("[TRENDING] Decision Dashboard")
    ...
    **Compared with 14 past simulations for similar holiday PLP tests:**
    ...
    with col1:
        if st.button("[CLIPBOARD] Export to Jira", use_container_width=True):
    ...
    with st.expander("[GEAR] Configuration & Variables", expanded=False):
        st.markdown("""
    ...
    with st.expander("[ROCKET] Simulation Execution", expanded=False):
        st.markdown("""
    ...
    with st.expander("[CHART] Results & Decision", expanded=False):
        st.markdown("""
    ...
    with st.expander("[TARGET] Rollout Plan", expanded=False):
        st.markdown("""
    ...
    **Next Steps:**
    ...
    with col2:
        if st.button("[CHART] Excel Summary", use_container_width=True):
